...
| **Frontend** | [Integration with React](https://docs.langchain.com/oss/javascript/langchain/streaming/frontend) | Server-side only | Server-side only |
...
- **Sandbox-as-tool pattern**: Run individual operations in isolated sandboxes from [different providers](https://docs.langchain.com/oss/python/deepagents/sandboxes) while the agent runs outside, or run the full agent inside a sandbox
...
| File Read/Write/Edit | ✅ [`ls`, `read_file`, `write_file`, `edit_file`](/oss/javascript/deepagents/harness#virtual-filesystem-access) | ✅ [Read, write, edit](https://platform.claude.com/docs/en/agent-sdk/overview#capabilities) | ✅ Built-in |
...
| Agent runs in sandbox | ✅ [Can run in sandbox](/oss/javascript/python/deepagents/sandboxes#agent-in-sandbox-pattern) | ✅ [Can run in sandbox](https://platform.claude.com/docs/en/agent-sdk/hosting) | ✅ [Can run in sandbox](https://developers.openai.com/codex/security#os-level-sandbox) |
...
| ACP server (IDE integration) | ✅ [ACP server](/oss/javascript/deepagents/acp) (python only) | ✅ (third-party) | ✅ (third-party) |
| MCP server mode | [MCP endpoint through Agent Server](/langsmith/server-mcp#mcp-endpoint-in-agent-server) | ❌ | ✅ [`codex mcp-server`](https://developers.openai.com/codex/guides/agents-sdk) |
