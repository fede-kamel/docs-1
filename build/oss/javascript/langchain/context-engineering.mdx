...
- Return a [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use @[`ExtendedModelResponse`]; JavaScript: return `Command` directly).
- Use life-cycle hooks like `before_model`, `after_model`, or `wrap_tool_call` (for tool returns) to update the conversation history. See the [middleware documentation](/oss/javascript/langchain/middleware) for more details.
