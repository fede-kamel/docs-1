...
const traceLayerSchema = withLangGraph(z.string().optional(), {
  reducer: {
    fn: (_a: string | undefined, b: string) => b,
  },
  default: () => undefined,
});

/** Agent state: trace_layer uses last-wins (outer wins), messages use additive reducer. */
const customMiddlewareStateSchema = z.object({
  trace_layer: traceLayerSchema,
});
