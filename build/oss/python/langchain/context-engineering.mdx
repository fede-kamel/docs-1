...
- Return a [`Command`](https://reference.langchain.com/python/langgraph/types/Command) from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use [`ExtendedModelResponse`](https://reference.langchain.com/python/langchain/agents/middleware/types/ExtendedModelResponse); JavaScript: return `Command` directly).
