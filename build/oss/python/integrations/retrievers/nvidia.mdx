---
title: "NVIDIARetriever integration"
description: "Integrate with the NVIDIARetriever using LangChain Python."
---

`NVIDIARetriever` connects LangChain to a running [NVIDIA RAG Blueprint](https://docs.nvidia.com/ai-enterprise/rag-llm-operator/latest/overview.html) server and retrieves relevant documents via the `/v1/search` endpoint. It supports sync and async retrieval, reranking, query rewriting, and metadata filtering.
...
`NVIDIARetriever` requires a running NVIDIA RAG Blueprint server. Refer to the [NVIDIA RAG Blueprint documentation](https://docs.nvidia.com/ai-enterprise/rag-llm-operator/latest/overview.html) for deployment instructions. By default the server listens on `http://localhost:8081` and expects at least one ingested collection in its vector database.
...
For detailed documentation of all `NVIDIARetriever` features and configurations head to the [API reference](https://python.langchain.com/api_reference/nvidia_ai_endpoints/retrievers/langchain_nvidia_ai_endpoints.retrievers.NVIDIARetriever.html).
...
- [`NVIDIAEmbeddings` integration](/oss/python/integrations/text_embedding/nvidia_ai_endpoints)
- [NVIDIA RAG Blueprint documentation](https://docs.nvidia.com/ai-enterprise/rag-llm-operator/latest/overview.html)
