...
- **Node-style hooks** (`before_agent`, `before_model`, `after_model`, `after_agent`): Return a dict directly. The dict is merged into the agent state.
- **Wrap-style hooks** (`wrap_model_call`): Return @[`ExtendedModelResponse`] with a @[`Command`] to inject state updates alongside the model response. Use this when you need to track or update state based on logic that runs during the model call, such as summarization trigger points, usage metadata, or custom fields calculated from the request or response.
...
Return `ExtendedModelResponse` from `wrap_model_call` when you need to apply state updates:
...
const usageTrackingStateSchema = z.object({
  last_model_call_tokens: z.number().optional(),
});
...
const trackUsage = createMiddleware({
  ...
  wrapModelCall: async (request, handler) => {
    ...
    return new Command({ update: { last_model_call_tokens: 150 } });
  },
});
```
...
- **Inner commands propagate outward:** Outer middleware always receives a plain `ModelResponse` from `handler()`. `ExtendedModelResponse` is unwrapped at composition boundaries.
...
- **Retry-safe:** When outer middleware retries by calling `handler()` again, accumulated inner commands are cleared and re-collected.
...
const outerMiddleware = createMiddleware({
  ...
  wrapModelCall: async (request, handler) => {
    const response = await handler(request);
    return new Command({ update: { outer_key: "val" } });
  },
});
...
const innerMiddleware = createMiddleware({
  ...
  wrapModelCall: async (request, handler) => {
    const response = await handler(request);
    return new Command({ update: { inner_key: "val" } });
  },
});
