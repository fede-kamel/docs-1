...
- Return a @[`Command`] from `wrap_model_call` / `wrapModelCall` to inject state updates from the model call layer (Python: use @[`ExtendedModelResponse`]; JavaScript: return `Command` directly).
- Use life-cycle hooks like `before_model` or `after_model` to update the conversation history. See the [middleware documentation](/oss/langchain/middleware) for more details.
