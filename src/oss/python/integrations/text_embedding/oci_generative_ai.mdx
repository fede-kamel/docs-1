...
# Index architecture diagrams, charts, and technical images
diagrams = [
...
query_vector = embeddings.embed_query("show me the database relationships")

# Find matching diagram
similarities = [
    np.dot(query_vector, img_vec) /
    (np.linalg.norm(query_vector) * np.linalg.norm(img_vec))
    for img_vec in image_vectors
]
best_match = diagrams[np.argmax(similarities)]
# Returns: "database_schema.png"
```
